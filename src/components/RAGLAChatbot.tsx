...
const RAGLAChatbot = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      // Extract WhatsApp links from text to show as buttons
      const extraLinks: { label: string; href: string }[] = [];
      const waMatch = data?.text?.match(/https:\/\/wa\.me\/[0-9]+/g);
      if (waMatch) {
        waMatch.forEach((url: string) => {
          if (!extraLinks.some(l => l.href === url)) {
            extraLinks.push({ label: "Chat on WhatsApp", href: url });
          }
        });
      }
                              ...
                              {msg.links.map((link) => {
                                ...
                                if (isExternal) {
                                  return (
                                    <a
                                      ...
                                      className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.1em] px-4 py-2.5 rounded-xl transition-all shadow-lg active:scale-95 ${
                                        isWhatsApp 
                                ...
                                return (
                                  <Link
                                    ...
                                    to={link.href}
                                    onClick={() => setIsOpen(false)}
                                    className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.1em] border border-primary/30 bg-primary/5 hover:bg-primary/10 text-primary px-4 py-2.5 rounded-xl transition-all"
                                  >
                                    {link.label} <ChevronRight className="w-3 h-3" />
                                  </Link>
                                );
